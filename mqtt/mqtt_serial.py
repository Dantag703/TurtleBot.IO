...
broker = 'b1ac2601.emqx.cloud'
port = 1883
topics_set = ['/turtlebot/arm/set','/turtlebot/wheels/wheel1/set',
              '/turtlebot/wheels/wheel2/set', '/turtlebot/arm/get','/turtlebot/stab/set']
...
def subscribe(client: mqtt_client):
    def on_message(client, userdata, msg):
        ...
        if msg.topic == '/turtlebot/arm/set':
           arm_trayectory(msg)
        elif msg.topic == '/turtlebot/wheels/wheel1/set':
            speed1 = int(msg.payload.decode())
            dev = serial.Serial(com, 57600)
            dev.flushInput()
            cad = f"dxl 0 {speed1}\n"
            dev.write(cad.encode("ascii"))
            dev.close()
        elif msg.topic == '/turtlebot/wheels/wheel2/set':
            speed2 = int(msg.payload.decode())
            dev = serial.Serial(com, 57600)
            dev.flushInput()
            cad = f"dxl 0 {speed2}\n"
            dev.write(cad.encode("ascii"))
            dev.close()
        elif msg.topic == '/turtlebot/arm/get':
            dev = serial.Serial(com, 57600, timeout=1000)
            dev.flushInput()   
            cad2="read"
            dev.write(cad2.encode("ascii"))
            ...
            serRead = dev.readline()
            print(serRead)
        elif msg.topic == '/turtlebot/stab/set':
